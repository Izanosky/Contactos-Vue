<template>
    ...
    <Card>
        <template #content>
            <DataTable :value="contactosFiltrados" paginator :rows="5" :rowsPerPageOptions="[5, 10, 20, 50]" stripedRows
                    ...
                    <div style="display: flex; justify-content: space-between; align-items: center; gap: 1rem;">
                        ...
                        <div style="display: flex; gap: 0.5rem; align-items: center;">
                            <IconField>
                                ...
                                <InputText v-model="valor" placeholder="Buscar por nombre o email"
                                    style="width: 300px;" />
                            </IconField>
                            <Button @click="añadir()" severity="info" icon="pi pi-plus" rounded aria-label="Añadir"
                ...
                <Column field="acciones" header="Acciones" alignHeader="center" bodyClass="text-center">
                    <template #body="slotProps">
                        <div class="flex gap-1 justify-center">
                                ...
                                <Button @click="editar(slotProps.data.ID)" severity="success" icon="pi pi-pencil"
                                    rounded raised v-tooltip.top="'Editar'" />
...
const contactosFiltrados = computed(() => {
    const filtro = valor.value.trim().toLowerCase()
    if (!filtro) {
        return contactos.value;
    }
    
    return contactos.value.filter((contacto) =>
        contacto.nombre.toLowerCase().includes(filtro) || contacto.email.toLowerCase().includes(filtro)
    )
})
...
function borrar(ID) {
    confirm.require({
        ...
        accept: () => {
            ...
            console.log(contactosStore.contactos)
